...
import type { FullTextScreeningQueueAssignment } from '@/types';

type FullTextScreeningQueueTableProps = {
    assignments: FullTextScreeningQueueAssignment[];
    selectedAssignmentId: string | null;
};

export function FullTextScreeningQueueTable({
    assignments,
    selectedAssignmentId,
}: FullTextScreeningQueueTableProps) {
    if (assignments.length === 0) {
        return (
            <div className="rounded-lg border bg-card p-6 text-sm text-muted-foreground shadow-xs">
                No full-text assignments are available for this reviewer.
            </div>
        );
    }
                        ...
                        >
                            ...
                            <TableCell>
                                <Badge variant="outline">
                                    {assignment.source_alias ?? 'Artifact'}
                                </Badge>
                            </TableCell>
                            <TableCell>

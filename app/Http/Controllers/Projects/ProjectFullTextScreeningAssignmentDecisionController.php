...
class ProjectFullTextScreeningAssignmentDecisionController extends Controller
{
    public function store(
        Request $request,
        Project $project,
        ProjectScreeningAssignment $assignment,
        RecordProjectFullTextScreeningDecision $recordDecision,
    ): RedirectResponse {
        ...
        $recordDecision->handle(
            ...
            $data['decision'],
            $data['reason'],
            $request->boolean('artifact_inspected'),
            $this->lines($data['evidence'] ?? null),
            $this->lines($data['uncertainty'] ?? null),
            $this->lines($data['exclusion_basis'] ?? null),
        );

        Inertia::flash('toast', ['type' => 'success', 'message' => __('Full-text screening decision recorded.')]);

        return to_route('projects.full-text-screening.queue', [
            'project' => $project,

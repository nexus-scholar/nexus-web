...
final class ProjectFullTextScreeningQueueReadModel
{
    /**
    ...
    private function assignments(ProjectScreeningBatch $batch, User $actor): array
    {
        return DB::table('project_screening_assignments as assignments')
            ...
            ->leftJoin('project_full_text_items as full_text_items', 'full_text_items.id', '=', 'assignments.source_full_text_item_id')
            ...
            ->get([
                ...
                'works.venue_name',
                'decisions.decision',
                'full_text_items.source_alias',
                'full_text_items.artifact_type',
            ])
            ->map(fn (object $row): array => [
                ...
                'source_alias' => $row->source_alias ? (string) $row->source_alias : null,
                'artifact_type' => $row->artifact_type ? (string) $row->artifact_type : null,
                'decided_at' => $this->dateString($row->decided_at),
                'href' => route('projects.full-text-screening.queue', [$batch->project_id, 'assignment' => (string) $row->id], absolute: false),
            ])
            ->all();
    }
    ...
    {
        $assignment->loadMissing(['batch.project', 'sourceFullTextItem']);
        ...
        $decision = $assignment->screening_decision_id
            ...
            : null;
        $fullTextItem = $assignment->sourceFullTextItem;

        return [
            'id' => $assignment->id,
            'status' => $assignment->status->value,
            'status_label' => $this->assignmentStatusLabel($assignment->status->value),
            'decision_url' => route('projects.full-text-screening.assignments.decision', [$assignment->project_id, $assignment->id], absolute: false),
            ...
            'title_abstract_handoff' => [
                'decision' => $fullTextItem instanceof ProjectFullTextItem ? $fullTextItem->screening_decision : null,
                'decision_label' => $fullTextItem instanceof ProjectFullTextItem ? $this->decisionLabel($fullTextItem->screening_decision) : null,
                'reason' => $fullTextItem instanceof ProjectFullTextItem ? ($fullTextItem->metadata['screening_reason'] ?? null) : null,
            ],
            'artifact' => $fullTextItem instanceof ProjectFullTextItem ? [
                'id' => $fullTextItem->id,
                'status' => $fullTextItem->status->value,
                'status_label' => $fullTextItem->status->label(),
                'source_alias' => $fullTextItem->source_alias,
                'artifact_type' => $fullTextItem->artifact_type,
                'artifact_path' => $fullTextItem->artifact_path,
                'download_url' => $fullTextItem->artifact_path
                    ? route('projects.full-text.artifacts.show', [$fullTextItem->project_id, $fullTextItem->id], absolute: false)
                    : null,
                'completed_at' => $this->dateString($fullTextItem->completed_at),
                'metadata' => $fullTextItem->metadata ?? [],
            ] : null,
            'work' => [
